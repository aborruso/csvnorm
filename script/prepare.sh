...
reject_errors_file="${output_dir}/${base_name}_reject_errors.csv"
rm -f "${reject_errors_file}"
...
duckdb -c "copy (from read_csv('$input_file',store_rejects = true,sample_size=-1,all_varchar=true)) TO '/dev/null';copy (FROM reject_errors) to '${reject_errors_file}'"

# Check if there are any errors
if [ "$(wc -l <"${reject_errors_file}")" -gt 1 ]; then
	echo "Error: DuckDB encountered invalid rows while processing the CSV file."
	echo "Details of the errors can be found in: ${reject_errors_file}"
...
reject_rows=$(wc -l <"${reject_errors_file}" 2>/dev/null || echo 0)
if [ -f "${reject_errors_file}" ] && [ "$reject_rows" -le 1 ]; then
	rm -f "${reject_errors_file}"
fi
